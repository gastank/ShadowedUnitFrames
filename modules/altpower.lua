...
function AltPower:UpdateVisibility(frame)
	...
	local barType = GetUnitPowerBarInfoByID(barId)
	local visible = false

	if( barType ) then
		if( ( frame.unitType == "player" or frame.unitType == "pet" ) or not barType.hideFromOthers ) then
			visible = true
		elseif( barType.showOnRaid and ( UnitInRaid(frame.unit) or UnitInParty(frame.unit) ) ) then
			visible = true
		end
	end
